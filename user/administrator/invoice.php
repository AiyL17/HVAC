...
// Fetch all completed appointments with a price (i.e., invoice-worthy)
...
WHERE
    a.app_status_id = 3 AND a.app_price IS NOT NULL AND a.app_price != ''
ORDER BY
    a.app_created DESC");
$query->execute();
$allInvoices = $query->fetchAll(PDO::FETCH_OBJ);
...
foreach ($allInvoices as $inv) {
    ...
    // Check search query (search in customer name)
    $searchMatch = true;
    if (!empty($searchQuery)) {
        $customerName = trim($inv->customer_fname . ' ' . $inv->customer_mname . ' ' . $inv->customer_lname);
        $searchMatch = stripos($customerName, $searchQuery) !== false;
    }
...
// Get total number of filtered invoices
$totalRecords = count($filteredInvoices);
$totalPages = ceil($totalRecords / $recordsPerPage);

// Slice invoices for current page
$invoices = array_slice($filteredInvoices, $startFrom, $recordsPerPage);
?>
<style>
    ...
    /* Mobile view - Remove card borders and add shadows */
    @media (max-width: 767.98px) {
        .card.border-start {
        ...
        /* Add shadow to cards in mobile view */
        .card {
...
<h3 style="margin-top: -10px; text-align:left;"><?= ucfirst($_GET['page']) ?></h3>
    <!-- Filters and Search -->
            ...
            <div class="row g-3" style="border-radius: 12px;">
                ...
                <div class="col-md-6 filter-search-column" style="padding-left: 190px;">
                    ...
                    <div class="input-group filter-search-input" style="width: 300px;">
                        ...
                        <input type="text" id="customerNameSearch" class="form-control" style="font-weight: 400; color: #505050;" placeholder="Search by customer name..." 
        ...
        <div class="d-none d-lg-block">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td><?= 'INV-' . str_pad($inv->app_id, 3, '0', STR_PAD_LEFT) ?></td>
                            <td><?= htmlspecialchars(trim($inv->customer_fname . ' ' . $inv->customer_mname . ' ' . $inv->customer_lname)) ?></td>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-sm btn-primary view-invoice-btn"
                                    ...
                                    data-invoice="<?= 'INV-' . str_pad($inv->app_id, 3, '0', STR_PAD_LEFT) ?>"
                                    data-customer="<?= htmlspecialchars(trim($inv->customer_fname . ' ' . $inv->customer_mname . ' ' . $inv->customer_lname)) ?>"
                                    data-house-building-street="<?= htmlspecialchars($inv->house_building_street) ?>"
                                    data-barangay="<?= htmlspecialchars($inv->barangay) ?>"
                                    data-municipality-city="<?= htmlspecialchars($inv->municipality_city) ?>"
                                    data-province="<?= htmlspecialchars($inv->province) ?>"
                                    data-zip-code="<?= htmlspecialchars($inv->zip_code) ?>"
                                    ...
                                    data-appliance="<?= htmlspecialchars($inv->appliances_type_name) ?>"
                                    data-technician="<?= htmlspecialchars(trim($inv->tech_fname . ' ' . $inv->tech_mname . ' ' . $inv->tech_lname)) ?>"
                                    data-technician2="<?= htmlspecialchars(trim($inv->tech2_fname . ' ' . $inv->tech2_mname . ' ' . $inv->tech2_lname)) ?>"
                                    data-status="<?= $status ?>"
                                    data-desc="<?= htmlspecialchars($inv->app_desc) ?>"
                                    data-amount="₱<?= number_format($inv->app_price, 2)?>"
                                    data-justification="<?= htmlspecialchars($inv->app_justification) ?>"
                                    data-comment="<?= htmlspecialchars($inv->app_comment ?? '') ?>"
                                    data-rating="<?= $inv->app_rating ?>"
                                    data-completed-at="<?= $inv->app_completed_at ?>"
                                >View</button>
                                <?php
                                // Combine individual address fields into a single address string with proper fallbacks
                                $addressParts = [
                                    !empty($inv->house_building_street) ? trim($inv->house_building_street) : '',
                                    !empty($inv->barangay) ? trim($inv->barangay) : '',
                                    !empty($inv->municipality_city) ? trim($inv->municipality_city) : '',
                                    !empty($inv->province) ? trim($inv->province) : '',
                                    !empty($inv->zip_code) ? trim($inv->zip_code) : ''
                                ];
                                $customerAddress = implode(', ', array_filter($addressParts, function($part) {
                                    return !empty($part);
                                }));
                                
                                // Fallback if no address parts are available
                                ...
                                <button type="button" class="btn btn-sm btn-success ms-1 download-invoice-btn" title="Download Invoice"
                                    ...
                                    data-servicetype="<?= htmlspecialchars($inv->service_type_name) ?>"
                                    data-servicetypeid="<?= $inv->service_type_id ?>"
                                    data-appliancetype="<?= htmlspecialchars($inv->appliances_type_name) ?>"
                                    data-technician="<?= htmlspecialchars(trim($inv->tech_fname . ' ' . $inv->tech_mname . ' ' . $inv->tech_lname)) ?>"
                                    data-technician2="<?= htmlspecialchars(trim($inv->tech2_fname . ' ' . $inv->tech2_mname . ' ' . $inv->tech2_lname)) ?>"
                                    data-status="<?= $status ?>"
                                    data-desc="<?= htmlspecialchars($inv->app_desc) ?>"
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5">
                                <div class="empty-state">
                                    ...
                                    <p class="text-muted mb-0">There are no invoices matching your current filters.</p>
                                    <?php if (!empty($statusFilter) && $statusFilter !== 'All' || !empty($searchQuery)): ?>
                                        <div class="mt-3">
                                            <a href="?page=invoice" class="btn btn-sm btn-outline-primary">
    ...
    <div class="d-lg-none" id="mobileInvoiceCardsContainer">
            ...
            ?>
            <div class="card mb-3 border-start border-4 border-<?= $statusClass ?>">
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h6 class="card-title mb-1 fw-bold text-primary"><?= 'INV-' . str_pad($inv->app_id, 3, '0', STR_PAD_LEFT) ?></h6>
                            <p class="card-text mb-1 text-muted small">
                                <i class="bi bi-person me-1"></i>
                                <?= htmlspecialchars(trim($inv->customer_fname . ' ' . $inv->customer_mname . ' ' . $inv->customer_lname)) ?>
                            </p>
                        </div>
            ...
            <?php if ($filteredCount === 0): ?>
                <div class="empty-state-mobile text-center py-5">
                    ...
                    <h5 class="text-muted mt-3">No Invoices Found</h5>
                    <p class="text-muted mb-0 px-3">There are no invoices matching your current filters.</p>
                    <?php if (!empty($statusFilter) && $statusFilter !== 'All' || !empty($searchQuery)): ?>
                        <div class="mt-4">
                            <a href="?page=invoice" class="btn btn-outline-primary">
        ...
        <nav aria-label="Page navigation" class="text-center">
            <ul class="pagination p-2 justify-content-center pagination-sm">
                <li class="page-item <?= $page === 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= $page > 1 ? '?page=invoice&status=' . urlencode($statusFilter) . '&search=' . urlencode($searchQuery) . '&page_num=' . ($page - 1) : '#' ?>" aria-label="Previous">
                        &lt;
                    </a>
                </li>
                ...
                if ($totalPages <= $maxVisiblePages) {
                    ...
                    for ($i = 1; $i <= $totalPages; $i++) {
                        ...
                        echo '<a class="page-link" href="?page=invoice&status=' . urlencode($statusFilter) . '&search=' . urlencode($searchQuery) . '&page_num=' . $i . '">' . $i . '</a>';
                        echo '</li>';
                    }
                } else {
                    // Show current page and adjacent pages with ellipsis
                    ...
                    if ($startPage > 1) {
                        ...
                        echo '<a class="page-link" href="?page=invoice&status=' . urlencode($statusFilter) . '&search=' . urlencode($searchQuery) . '&page_num=1">1</a>';
                        echo '</li>';
                        if ($startPage > 2) {
                    ...
                    if ($endPage < $totalPages) {
                        ...
                        echo '<li class="page-item">';
                        echo '<a class="page-link" href="?page=invoice&status=' . urlencode($statusFilter) . '&search=' . urlencode($searchQuery) . '&page_num=' . $totalPages . '">' . $totalPages . '</a>';
                        echo '</li>';
                    }
                }
                ?>
                ...
                <li class="page-item <?= $page === $totalPages ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= $page < $totalPages ? '?page=invoice&status=' . urlencode($statusFilter) . '&search=' . urlencode($searchQuery) . '&page_num=' . ($page + 1) : '#' ?>" aria-label="Next">
                        &gt;
                    </a>
                </li>
            </ul>
...
<div class="modal fade" id="invoiceModal" tabindex="-1" aria-labelledby="invoiceModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content round_lg">
      ...
      <div class="modal-body p-4" style="border-radius: 12px;">
        <div class="row">
          <div class="col-md-6">
            <div class="mb-3">
        ...
        <div class="mt-3" id="feedbackSection" style="display: none;">
          ...
          <div class="p-3 bg-light rounded border">
            ...
            <div>
              ...
              <p id="modalComment" class="mb-0 mt-1"></p>
            </div>
          </div>
        </div>
      </div>
...
<!-- Add a hidden printable invoice template -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.view-invoice-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            modalInvoiceNo.textContent = btn.getAttribute('data-invoice');
            modalCustomer.textContent = btn.getAttribute('data-customer');
            // Construct address from dissected fields
            var houseBuildingStreet = btn.getAttribute('data-house-building-street') || '';
            var barangay = btn.getAttribute('data-barangay') || '';
            var municipalityCity = btn.getAttribute('data-municipality-city') || '';
            var province = btn.getAttribute('data-province') || '';
            var zipCode = btn.getAttribute('data-zip-code') || '';
            
            // Combine address components
            var addressParts = [];
            if (houseBuildingStreet.trim() !== '') addressParts.push(houseBuildingStreet);
            if (barangay.trim() !== '') addressParts.push(barangay);
            if (municipalityCity.trim() !== '') addressParts.push(municipalityCity);
            if (province.trim() !== '') addressParts.push(province);
            if (zipCode.trim() !== '') addressParts.push(zipCode);
            
            var address = addressParts.join(', ');
            modalAddress.textContent = address || 'Not Specified';
            modalDate.textContent = btn.getAttribute('data-date');
            modalServiceType.textContent = btn.getAttribute('data-servicetype');
            modalStatus.textContent = btn.getAttribute('data-status');
            modalDesc.textContent = btn.getAttribute('data-desc');
            modalAmount.textContent = btn.getAttribute('data-amount');
            modalJustification.textContent = btn.getAttribute('data-justification');
            
            // Handle technician display
            var technician = btn.getAttribute('data-technician');
            var technician2 = btn.getAttribute('data-technician2');
            
            modalTechnician.textContent = technician || 'Not assigned';
            ...
            if (comment && comment.trim() !== '' && comment !== 'No Comment' && rating) {
                // Display star rating
                var stars = '';
                for (var i = 1; i <= 5; i++) {
                ...
                modalRating.innerHTML = stars + ' (' + rating + '/5)';
            ...
            const serviceTypeId = btn.getAttribute('data-servicetypeid');
            if (serviceTypeId) {
            ...
            var completedAt = btn.getAttribute('data-completed-at');
            if (completedAt) {
                var completedDate = new Date(completedAt);
                var formattedDate = completedDate.toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                ...
                document.getElementById('modalInvoiceDate').textContent = formattedDate;
            }
        });
    });
    ...
    document.querySelectorAll('.download-invoice-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            var paymentStatus = btn.getAttribute('data-status');
            var isPaid = paymentStatus === 'Paid';
            var windowTitle = isPaid ? 'Official Receipt' : 'Invoice';
            ...
            if (isPaid) {
                ...
                var technicianName = btn.getAttribute('data-technician') || 'Not assigned';
                var technician2Name = btn.getAttribute('data-technician2');
                    ...
                    <body>
                      <div class="container">
                          ...
                          <div class="left-bordered">
                            ...
                            <table class="settlement-table">
                              <tr>
                                ...
                                <td style="width: 50%; border-left: 1px solid #000;"><strong>${btn.getAttribute('data-servicetype')}</strong></td>
                              </tr>
                              <tr>
                                <td>Appliances Type:</td>
                                <td style="border-left: 1px solid #000;"><strong>${btn.getAttribute('data-appliancetype')}</strong></td>
                              </tr>
                              <tr>
                                <td>Service by:</td>
                              ...
                              <tr>
                                <td>Date & Time:</td>
                                <td style="border-left: 1px solid #000;"><strong>${btn.getAttribute('data-date')}</strong></td>
                              </tr>
                              <tr>
                                <td></td>
                              ...
                              <tr>
                                ...
                                <td style="border-left: 1px solid #000;"><strong>${btn.getAttribute('data-amount')}</strong></td>
                              </tr>
                              <tr>
                        ...
                        <div class="right">
                          ...
                          <p style="margin-bottom: 8px;">RECEIVED from <span class="customer-name-line"><strong>${btn.getAttribute('data-customer')}</strong></span></p>
                          <p style="margin-bottom: 8px;">with address at <span class="address-line"><strong>${btn.getAttribute('data-address') || 'Address not specified'}</strong></span></p>
                          ...
                          <p style="margin-bottom: 8px;"><span class="business-style-line"></span> the amount of <span class="filled-line-medium"><strong>${convertAmountToWords(btn.getAttribute('data-amount'))}</strong></span></p>
                          <p style="margin-bottom: 8px;"><span class="filled-line-long"></span> (<span class="filled-line-med"><strong>${btn.getAttribute('data-amount')}</strong></span>)</p>
                          <p style="margin-bottom: 8px;">as (✓) full / ( ) partial payment for <span class="filled-line-long"><strong>${btn.getAttribute('data-servicetype')} - ${btn.getAttribute('data-appliancetype')}</strong></span></p>
                          <p style="margin-bottom: 15px;"><span class="filled-line-lo"><strong>${btn.getAttribute('data-desc')}</strong></span></p>
                          <div class="flex-space">
                          ...
                          <p><strong>No.</strong> <u><strong>${btn.getAttribute('data-invoice')}</strong></u></p>
                ...
                // Use the regular invoice format for unpaid invoices
                var technician = btn.getAttribute('data-technician') || 'Not assigned';
                var technician2 = btn.getAttribute('data-technician2');
                ...
                // Get status and add appropriate styling
                var invoiceStatus = btn.getAttribute('data-status');
                        ...
                        <div class="invoice-container">
                            ...
                            <div class="invoice-header">
                                ...
                                <div class="header-info">
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">${btn.getAttribute('data-invoice')}</div>
                                    </div>
                                    <div class="info-item">
                                    ...
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">${btn.getAttribute('data-amount')}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="billing-section">
                                <div class="billing-block">
                                    ...
                                    <div class="customer-name">${btn.getAttribute('data-customer')}</div>
                                    <div class="customer-address">${btn.getAttribute('data-address') || 'Address not specified'}</div>
                                </div>
                                <div class="billing-block">
                            ...
                            <div class="items-section">
                                <table class="items-table">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                <strong>${btn.getAttribute('data-servicetype')}</strong><br>
                                                <span style="color: #666; font-size: 13px;">Equipment: ${btn.getAttribute('data-appliancetype') || 'Not specified'}</span>
                                                <div class="service-description">${btn.getAttribute('data-desc')}</div>
                                            </td>
                                            <td>${btn.getAttribute('data-date')}</td>
                                            <td>
                                                ${btn.getAttribute('data-technician') || 'Not assigned'}
                                                ${btn.getAttribute('data-technician2') && btn.getAttribute('data-technician2').trim() !== '' ? '<br>' + btn.getAttribute('data-technician2') : ''}
                                            </td>
                                            <td>${btn.getAttribute('data-amount')}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    function performSearch() {
        const searchValue = customerNameSearch.value.trim();
        const currentStatus = new URLSearchParams(window.location.search).get('status') || 'All';
        
        // Build the URL with current status and search query
        const url = new URL(window.location.href);
        url.searchParams.set('page', 'invoice');
        url.searchParams.set('status', currentStatus);
        url.searchParams.set('page_num', '1'); // Reset to first page on search
    ...
    customerNameSearch.addEventListener('input', function() {
        ...
        // Set new timeout for 500ms delay (debounce)
        searchTimeout = setTimeout(performSearch, 500);
    });
    
    // Also trigger search on Enter key
    customerNameSearch.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            clearTimeout(searchTimeout);
            performSearch();
        }
